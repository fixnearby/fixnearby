...
const Verifyotp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate API call
    try {
      const response = await axiosInstance.post("/repairer/verify-otp",{phone,otp});
      if (response.status ===200 || response.status === 201) {
        toast.success('OTP verified successfully!');
        setOtp(''); 
        navigate("/repairer/signup",{ state: { phone } });
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mt-8 space-y-2">
          <p className="text-sm text-gray-600">
            ...
            <a href="/repairer/login" className="text-blue-600 hover:text-purple-600 font-semibold transition-colors">
              Sign in
            </a>
          </p>
